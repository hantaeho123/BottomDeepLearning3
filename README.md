...
### 🧠 AI & Computer Vision
* **Vision Models:** YOLO11n, MediaPipe Pose/Hands, TrackNetV3
...
* **10개 세부 평가 지표:**
  * **Ready (준비):** 팔꿈치 각도(18°~70°), 손목 높이, 스탠스 너비, 팔꿈치 높이
  * **Backswing (백스윙):** 골반/어깨 회전율, 손목 깊이, 팔꿈치 들림 비, L자 팔 각도(60°~110°)
  * **Impact & Follow:** 팔 뻗음 각도(140°~180°), 손목 높이 비율, 팔로우스루 시 손목/팔꿈치 x좌표 교차 검증
...
* **시도:** TrackNetV3와 YOLO를 결합해 셔틀콕의 타점과 낙하점을 포착하고 이상적인 스윙 궤적을 시각화하고자 했습니다.
* **결론:** 일반 체육관의 하얀 벽으로 인한 보호색 현상(객체 소실), 조명 노이즈, 정적 객체 오탐지 등 현존하는 비전 AI가 겪는 통제되지 않은 환경에서의 명백한 기술적 한계를 확인했습니다. 무리한 도입 대신, **스윙 자세 교정 알고리즘의 정확도를 높이는 데 서비스 역량을 집중**했습니다.
...
| 이름 | 역할 분담 |
| :---: | :--- |
| **노은서** | **팀장/PM**, 스윙 자세 교정 알고리즘, 셔틀콕 궤적 분석 R&D |
| **김민지** | **프론트엔드**, 스윙 자세 교정 알고리즘 |
| **이원호** | **백엔드**, LLM 종합 리포트 및 RAG 파이프라인 구축 |
| **권주은** | **백엔드**, DB 및 인프라 설계 |
| **한태호** | **그립 알고리즘**, YOLO 모델 파인튜닝 |

<br>
<p align="center">
  <i>멋쟁이사자처럼 AICV 3기 종합 프로젝트</i>
</p>
